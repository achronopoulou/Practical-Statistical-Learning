...
The *squared* Euclidean distance $\|u-v\|_2^2$ is not a metric (it fails the triangle inequality), but it is the quantity minimized by $K$-means. The two distances yield the same nearest-center assignments.
...
The Python code for this chapter is available here <a href="code/ch12_PythonCode.html"> <b>Python Code for Clustering</b> </a>. It reproduces the main R examples (kmeans, hclust, and the spectral clustering demo).
...
Note that the **total distance** can be broken down into
\begin{align*}
...
```{r}    
    ...
    # if we use multiple starting points and pick the best one
...
Note that the clusters are typically **initialized randomly**.  However, this algorithm does not guarantee a global minimum, although it still leads to a local minimizer.
...
An alternative approach is the $K$-**medoids** method. We need to replace the **first step** (updating the centers) by searching for *the one* observation that minimizes the distance to all others in the cluster, i.e. 
...
So, to determine the **optimal $K$** with the **gap statistic**, we plot the gap statistic values for different values of $K$. The optimal $K$ is determined either by identifying the highest gap statistic or, in a sequential approach, by selecting the first $K$ where its gap statistic exceeds that of $K+1$.
...
A rule of thumb taken from Kaufman and Rousseeuw (1990); see also Struyf, Hubert and Rousseeuw (1997), "_Clustering in an Object-Oriented Environment_":
...
For each cluster obtained in _Step 1_ (the clusters found on the test set $B$), examine every pair of observations within that cluster. Check whether those pairs remain in the same cluster under the assignment from _Step 2_. Calculate the average co-membership accuracy for each test cluster, indicating how often pairs of observations remain together.
...
* **Ward Linkage**: merge the pair of clusters whose combination increases the within-cluster sum of squares the least. This linkage is the closest hierarchical analogue of the $K$-means objective.
...
Self-organizing Map (SOM) is another popular clustering method. The main difference between SOM and $K$-means is that the cluster means of a SOM have a geometric relationship preserving the geometry of the underlying data. An SOM starts with **Input Data**: $\{x_i\}_{i=1}^{n}$, with each $x_i \in \mathbb{R}^{p}$. We then randomly initialize the **Cluster Means**, i.e. creating a grid of centers $w_{ij} \in \mathbb{R}^{p}$, $i=1, \ldots, q_1$, $j=1,\ldots, q_2$. These are similar to the centers in a $K$-means algorithm, but they also preserve some geometric relationship on the grid. Then, we pick a **Learning Rate**: $\alpha \in[0,1]$. This controls how fast the $w_{ij}$'s are updated, and it will decrease progressively. We also need to choose the **Radius**: $r$. This controls how many $w_{ij}$'s will be updated at each
...
3. Update each $w_{ij}$ based on the formula
\[w_{ij} \leftarrow w_{ij} + \alpha \, h(w_{*}, w_{ij}, r)\, (x_k - w_{ij})\]
where $h$ is a neighborhood function on the *grid* (typically depending on the grid distance between units and the current radius $r$). Units close to the winner on the map are pulled toward $x_k$; units outside the neighborhood are left unchanged.

4. Decrease the value of $\alpha$ and $r$.
...
Each of them has some unique properties:<br>
...
3. Compute the smallest $k$ eigenvectors, denote as $\mathbf{V}_{n \times k}$. (For $\mathbf{L}_{sym}$, a common extra step is to row-normalize $\mathbf{V}$ before clustering.)
...
```{r fig.width=14, fig.height=4, out.width = '95%'}
  ...
  par(mfrow=c(1,4))
  plot(f$vectors[, 200], type = "l", ylab = "eigenvector", ylim = c(-0.15, 0.15))
  plot(f$vectors[, 199], type = "l", ylab = "eigenvector", ylim = c(-0.15, 0.15))
  plot(f$vectors[, 198], type = "l", ylab = "eigenvector", ylim = c(-0.15, 0.15))
  plot(f$vectors[, 197], type = "l", ylab = "eigenvector", ylim = c(-0.15, 0.15))
```
...
```{r fig.width=6, fig.height=6, out.width = '35%'}
  ...
  # W is not necessarily symmetric
  W = pmax(W, t(W))
