...
The python version of the example below was written by our TA Tommy Tang and can be found here: <a href="data/week3/W3_VSReg_python.html"> <b>Python Version</b> </a> <br>
...
full.model = lm(Y[-test.id]~., data = stu_performance[-test.id, names(stu_performance) != "Grades"])
```
...
Ytest.pred = predict(full.model, newdata= stu_performance[test.id, names(stu_performance) != "Grades"]);
...
Fit a set of ridge regression models using `glmnet` with the default $\lambda$ sequence. Selecting $\alpha=0$ to run a **ridge**
<br>

We  plot the fitted output of the ridge regression. We can use the `xvar` argument to decide what is plotted on the x-axis. We can also label the curves with the variable index numbers simply by setting `label = TRUE`.

<br>

```{r}
ridge.lm = glmnet(X[-test.id, ], Y[-test.id], alpha = 0)
plot(ridge.lm , label = TRUE, xvar = "lambda")
```
<br>
...
```{r}
length(ridge.lm$lambda)
## remove the length function to see the lambda values
```
...
```{r}
dim(ridge.lm $beta)       ## coefficients for  non-intercept predictors
length(ridge.lm $a0)      ## intercept


## You can also retrieve coefficients (including intercept) using coef(myridge)
dim(coef(ridge.lm ))

## The two coefficient matrices should be the same
sum((coef(ridge.lm ) - rbind(ridge.lm $a0, ridge.lm $beta))^2)
```
...
## Compute the "intercept" by hand:
mean.ridge = apply(X[-test.id, ], 2, mean)  ## mean vector for training X
mean(Y[-test.id]) - sum(mean.ridge * ridge.lm $beta[, k])

## Extract intercept from the estimated coefficients
ridge.lm$a0[k]            ## intercept for lambda = myridge$lambda[k]

## Both approaches give the same value
sum((mean(Y[-test.id]) - mean.ridge %*% ridge.lm $beta  - ridge.lm $a0)^2)

```

<br>

We can also  choose `xvar="dev"` to _plot the fitted coefficients against the percent deviance_. This is the percent of deviance explained on the training data, and is a measure of complexity of the model. We can see that the coefficients do not change much after a point the but the coefficients start blowing up a bit.

<br>

```{r}
plot(ridge.lm, xvar = "dev", label = TRUE)
```


<br><br>

#### Cross-Validation & Path Plot

<br>
If we want to use cross-validation to choose $\lambda$, then we can use the function `cv.glmnet`. This function is using a $K$-fold cross validation method to obtain the "optimal lambda". The `cv.glmnet` has the attribute `nfolds` to select the number of folds we prefer, the option `foldid` to specify our own folds, and `type.measure` to choose the loss function (`deviance` or `mse` for squared loss, and `mae` for mean absolute error).

<br>
```{r}
ridge.cv = cv.glmnet(X[-test.id, ], Y[-test.id], alpha = 0) 
## the default in nfolds is 10
```
<br>
...
As a side note, the `cv.glmnet` function allows parallel computing for large scale problems.

The results are stored in

`ridge.cv$cvm`   mean CV errors
`ridge.cv$cvsd`: standard errors of CVm

<br>
We can also plot the result of the `cv.glmnet`:
...
The CV performance continues to drop when $\lambda$ gets smaller. So, we provide `R` with a $\lambda$ sequence dense in $[-2, 2]$:
<br>

```{r}
lambda.seq = exp(seq(-2, 4, length=100))
ridge.cv.seq = cv.glmnet(X[-test.id,], Y[-test.id], alpha=0, lambda=lambda.seq)  
plot(ridge.cv.seq)
```


We can still use the `coef` and `predict` methods for `cv.glmnet` objects in the same way as for  the `glmnet`. In this case, we also have two special strings supported by $s$ (the values of $\lambda$ requested):

* `lambda.min`: the value of lambda that achieves the minimum MSE (i.e. `cvm` -- the left dashed vertical line)

* `lambda.1se`: the largest value of lambda (i.e., the largest regularization, the smallest df) whose MSE (i.e. `cvm`) is within one-standard-error of the `cvm` of `lambda.min` -- i.e. the smallest MSE (-- the left dashed vertical line).
...
```{r}
## Find lambda.min
ridge.cv.seq$lambda.min
ridge.cv.seq$lambda[which.min(ridge.cv.seq$cvm)]
```

```{r}
## Find lambda.1se
ridge.cv.seq$lambda.1se
tmp.id = which.min(ridge.cv.seq$cvm)
max(ridge.cv.seq$lambda[ridge.cv.seq$cvm < ridge.cv.seq$cvm[tmp.id] + ridge.cv.seq$cvsd[tmp.id]])
```
...
Next we apply the two ridge regression models—one with `lambda.min` and one with `lambda.1se` on a new _test_ data set and report the corresponding **mean squared prediction errors**:
<br>
```{r}
ridge.lambda.pred = glmnet(X[-test.id,], Y[-test.id], alpha=0, lambda = lambda.seq)
```
...
Ytest.ridge.min.pred = predict(ridge.lambda.pred, s = ridge.cv.seq$lambda.min, newx=X[test.id,])
mean((Ytest.ridge.min.pred - Y[test.id])^2)
```

```{r}
## Using lambda.1se
Ytest.ridge.1se.pred = predict(ridge.lambda.pred, s = ridge.cv.seq$lambda.1se, newx=X[test.id,])
mean((Ytest.ridge.1se.pred - Y[test.id])^2)
```
...
<br>
If we want to use LASSO instead of Ridge, we need to choose $\alpha=1$ in the `glmnet` function. In this case, we have
<br>

```{r}
lasso.lm = glmnet(X[-test.id,], Y[-test.id], alpha = 1)
```
...
```{r}
par(mfrow = c(1, 2))
plot(lasso.lm, label=TRUE, xvar = "norm")   ## norm is the default value for xvar
plot(lasso.lm, label=TRUE, xvar = "lambda")
par(mfrow=c(1,1))
```
<br>
...
<br>
As with the ridge regression example, we can still use cross-validation by calling the `cv.glmnet`:
<br>

```{r}
lasso.cv = cv.glmnet(X[-test.id, ], Y[-test.id], alpha = 1)
plot(lasso.cv)
```
<br>

From the `cv.glmnet` object we can still compute `lambda.min` and `lambda.1se` in the same way as before:
...
```{r}
lasso.min.pred.coef = predict(lasso.lm, s=lasso.cv$lambda.min, type="coefficients");
lasso.1se.pred.coef = predict(lasso.lm, s=lasso.cv$lambda.1se, type="coefficients");
cbind(lasso.min.pred.coef, lasso.1se.pred.coef);
```

```{r}
## number of variables selected (including the intercept)
sum(lasso.1se.pred.coef != 0)
sum(lasso.min.pred.coef != 0)
```

```{r}
## names of selected non-intercept variables
row.names(lasso.1se.pred.coef)[which(lasso.1se.pred.coef != 0)[-1]]
row.names(lasso.min.pred.coef)[which(lasso.min.pred.coef != 0)[-1]]
...
```{r}
lasso.lambda.pred = glmnet(X[-test.id, ], Y[-test.id], alpha = 1)
```

```{r}
## Using the lambda.min
Ytest.lasso.min.pred = predict(lasso.lambda.pred, s = lasso.cv$lambda.min, newx = X[test.id,])
mean((Ytest.lasso.min.pred - Y[test.id])^2)
```


```{r}
## Using lambda.1se
Ytest.lasso.1se.pred = predict(lasso.lambda.pred, s = lasso.cv$lambda.1se, newx=X[test.id,])
mean((Ytest.lasso.1se.pred - Y[test.id])^2)
```
...
```{r}
lasso.coef.1se = predict(lasso.lambda.pred, s = lasso.cv$lambda.1se, type="coefficients")
variables.lasso = row.names(lasso.coef.1se)[which(lasso.coef.1se != 0)[-1]]
variables.lasso
```

```{r}
tmp.X = X[ , colnames(X) %in% variables.lasso]
lasso.refit = coef( lm( Y[-test.id] ~ tmp.X[-test.id ]) )
Ytest.pred = lasso.refit[1] + tmp.X[test.id] * lasso.refit[-1]
